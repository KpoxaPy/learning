...
public:
  ...
  size_t Get(TAirport airport) const {
    return stat_[static_cast<size_t>(airport)].second;
  }

  // добавить данный элемент
  void Insert(TAirport airport) {
    stat_[static_cast<size_t>(airport)].second++;
  }
  ...
  void EraseAll(TAirport airport) {
    stat_[static_cast<size_t>(airport)].second = 0;
  }
...
private:
  ...
  void InitStat() {
    for (size_t i = 0; i < stat_.size(); ++i) {
      stat_[i] = {static_cast<TAirport>(i), 0};
    }
  }
};
