...
private:
  ...
  void AdjustDegree(size_t degree) {
    if (coeffs_.size() - 1 < degree) {
...
public:
  class Index {
  public:
    Index(Polynomial& pol, size_t idx)
      : pol_(pol)
      , idx_(idx)
    {}
    ...
    Index& operator=(const T& value) {
      if (value != 0) {
        pol_.AdjustDegree(idx_);
        pol_.coeffs_[idx_] = value;
      } else {
        if (idx_ < pol_.coeffs_.size()) {
          pol_.coeffs_[idx_] = 0;
          pol_.Shrink();
        }
      }
    ...
    operator T() const {
      return const_cast<const Polynomial&>(pol_)[idx_];
    }
