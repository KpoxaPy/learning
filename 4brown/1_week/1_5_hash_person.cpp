...
struct Address {
  ...
  bool operator==(const Address& o) const {
    return building == o.building && city == o.city && street == o.street;
  }
};
...
struct AddressHasher {
  size_t operator()(const Address& a) const {
    const size_t x = 252'132'143'339;
    size_t city = sh(a.city);
    size_t street = sh(a.street);
    size_t building = ih(a.building);
    return x*x*city + x*street + building;
  }
...
struct PersonHasher {
  size_t operator()(const Person& p) const {
    const size_t x = 252'132'143'339;
    size_t name = sh(p.name);
    size_t height = ih(p.height);
    size_t weight = dh(p.weight);
    size_t address = ah(p.address);

    return x*x*x*name + x*x*height + x*weight + address;
  }
